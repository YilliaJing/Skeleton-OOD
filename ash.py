...
def ash_b(x, percentile):  # 65 80 85 90
    assert x.dim() == 4
    assert 0 <= percentile <= 100

    # print('ash-b  p = ' + str(percentile))
    b, c, h, w = x.shape
    ...
    t = x.view((b, c * h * w)).clone()
    v, i = torch.topk(t, k, dim=1)
    fill = s1 / k
    fill = fill.unsqueeze(dim=1).expand(v.shape)
    x = t.zero_().scatter_(dim=1, index=i, src=fill)
    return x


def ash_p(x, percentile):  # 65 80 85 90
    assert x.dim() == 4
    assert 0 <= percentile <= 100

    # print('ash-p  p = ' + str(percentile))
    b, c, h, w = x.shape

    n = x.shape[1:].numel()
    k = n - int(np.round(n * percentile / 100.0))  # torch.round()
    t = x.view((b, c * h * w)).clone()
    v, i = torch.topk(t, k, dim=1)
    x = t.zero_().scatter_(dim=1, index=i, src=v)
...
def ash_s(x, percentile):
    ...
    # print('ash-s  p = ' + str(percentile))

    # calculate the sum of the input per sample
    s1 = x.sum(dim=[1, 2, 3])
    n = x.shape[1:].numel()  # for each sample
    k = n - int(np.round(n * percentile / 100.0))
    t = x.view((b, c * h * w))  # [64, 2*256*400]
    v, i = torch.topk(t, k, dim=1)  # [64, 133120]
