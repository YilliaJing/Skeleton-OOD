...
# root_path = './'
root_path = '/home/cvlab/xj/Projs/HD-GCN2/data/ntu120/'
...
def align_frames(skes_joints, frames_cnt):
    ...
    aligned_skes_joints = np.zeros((num_skes, max_num_frames, 150), dtype=np.float32)  # 对齐帧数和joints  空缺的用0补
...
def alltest_dataset(skes_joints, label, performer, camera, save_path):
    # save all test dataset
    test_all_y = one_hot_vector(label)

    save_name_test = '/home/cvlab/additional-space/xj/data/ntu120/NTU120_all.npz'
    np.savez(save_name_test, x_test=skes_joints, y_test=test_all_y, label_id=label)


def split_dataset(skes_joints, label, performer, camera, evaluation,  save_path):
    train_indices, test_indices = get_indices(performer, camera, evaluation) # , label
...
def get_indices(performer, camera, evaluation='CS'):
    ...
    unseen_label = np.random.randint(0, 60, size=5)  # 0-59  随机生成5个unseen
    print('unseen: ')
    print(unseen_label)

    if evaluation == 'CS':  # Cross Subject (Subject IDs)    # 指定performer
...
if __name__ == '__main__':
    ...
    alltest_dataset(skes_joints, label, performer, camera, save_path)
